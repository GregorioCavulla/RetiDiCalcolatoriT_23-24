...
int main(int argc, char *argv[]) {
  ...
  printf("Operazioni: EO:Elimina caratteri numerici in un file, LF:Lista file "
         "carattere \n");
  while (gets(ok)) {
    if ((strcmp(ok, "EO") != 0) && (strcmp(ok, "LF") != 0)) {
      printf("scelta non disponibile\n");
      printf("Operazioni: EO:Elimina caratteri numerici in un file, LF:Lista "
             "file carattere \n");
      continue;
    }
    ...
    // richiesta conteggio caratteri nel file remoto
      ...
      if (out == NULL) {
        ...
        printf("Trovati %d file di testo con piu' di %d occorrenze nel nome "
               "del carattere %c\n",
