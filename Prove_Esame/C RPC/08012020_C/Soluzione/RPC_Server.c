 ...
 * ricevere la lista dei file di testo di un direttorio i cui nomi contengono
...
OutputFileList *lista_file_carattere_1_svc(Input *input, struct svc_req *rp) {
  ...
  while ((dd = readdir(dir)) != NULL && result.nFiles < 6) {
    ...
    if (fileNameL > 4 && fileName[fileNameL - 1] == 't' &&
        fileName[fileNameL - 2] == 'x' && fileName[fileNameL - 3] == 't' &&
