...
public class DiscoveryServer {
    ...
    public static void main(String[] args) {
        ...
        if ((args.length - 1) % 2 != 0 || args.length == 0) { // se il numero di argomenti è dispari o non ci sono
                                                              // argomenti
            System.out.println("Usage: java DiscoveryServer <portaDiscoveryServer> <file1 port1> ... <fileN portN>");
            System.exit(1);
        }

        PORT = Integer.parseInt(args[0]); // prendo la porta del discovery server

        if (PORT <= 1024 || PORT > 65535) { // controllo che la porta sia valida
            System.out.println("The discovery server port is not valid: " + args[0]);
            System.exit(2);
        }

        int validArgs = 0; // contatore per controllare che ci siano argomenti validi

        File fileCorr; // file corrente
        int portCorr; // porta corrente

        // per ogni coppia verifico che il primo argomento sia un file esistente e il
        // secondo una porta valida
        for (int i = 1; i < args.length; i = i + 2) {
            ...
            try {
                ...
                if (portCorr <= 1024 || portCorr > 65535) {
                    ...
                    for (int j = 1; j < args.length; j = j + 2) { // controllo che la porta non sia già stata usata
                        if (portCorr == Integer.parseInt(args[j + 1])) {
                            System.out.println("The following port is already in use: " + args[j + 1]);
                            continue;
                        }
                    }
                    continue;
