    ...
    for (;;)
    {
        ...
        if (FD_ISSET(udp_sd, &rset))
        {
            ...
            if (ris == 0)
            {
                ...
                { // Se il file temporaneo è stato creato correttamente
                    ...
                    while ((nread = read(fd_sorg_udp, &read_char, sizeof(char))) != 0)
                    {
                        ...
                        else
                        {
                            ...
                            if (read_char == ' ' || read_char == '\r' || read_char == '\n')
                            {
                                ...
                                if (strcmp(word_buffer, req.parola))
                                { // parola diversa
        ...
        if (FD_ISSET(listen_sd, &rset))
        {
            ...
            if (fork() == 0)
            {
                ...
                while ((nread = read(conn_sd, dir, sizeof(dir))) > 0)
                {
                    ...
                    char risp;
                    if ((dir1 = opendir(dir)) != NULL) // se il direttorio è aperto correttamente
                    {                                  // direttorio presente
                        risp = 'S';
                        printf("Invio risposta affermativa al client\n");
                        write(conn_sd, &risp, sizeof(char));
                        while ((dd1 = readdir(dir1)) != NULL) // leggo i file del direttorio
                        {
                            if (strcmp(dd1->d_name, ".") != 0 && strcmp(dd1->d_name, "..") != 0) // entro se non è . o ..
                            {
                                // build new path
                                sprintf(newDir, "%s/%s", dir, dd1->d_name); // cerco il file nel direttorio di secondo livello
                                if ((dir2 = opendir(newDir)) != NULL)       // se il direttorio è aperto correttamente (secondo livello)
                                {                                           // dir sec livello
                                    while ((dd2 = readdir(dir2)) != NULL)   // leggo i file del direttorio di secondo livello
                                    {
                                        if (strcmp(dd2->d_name, ".") != 0 &&
                                            strcmp(dd2->d_name, "..") != 0) // entro se non è . o ..
                                        {
                                            // build new path
                                            strcat(newDir, "/"); // aggiungo / al path per il file
                                            strcat(newDir, dd2->d_name);
                                            // se non è un direttorio, è un file da considerare
                                            if ((dir3 = opendir(newDir)) == NULL) // se non è un direttorio
                                            {                                     // file of sec livello
